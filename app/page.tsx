...
export default function Home() {
  ...
  return (
    <>
      {/* ── Mobile responsiveness (added 2026-08-26) — self-contained; overrides
          the external globals.css layout at phone/tablet widths. ───────────── */}
      <style>{`
        html,body{max-width:100vw;overflow-x:hidden}
        img,canvas,svg,video{max-width:100%}
        *{overflow-wrap:break-word}
        /* Validation rows: label + body. Stacks on mobile (was crushing the body text). */
        @media (max-width:860px){
          ...
          .standards-grid{grid-template-columns:repeat(3,1fr) !important} .manifest-grid{grid-template-columns:repeat(5,minmax(0,1fr)) !important}
        }
        @media (max-width:680px){
          .nav-links{display:none !important}
          .val-row{grid-template-columns:1fr !important;gap:.4rem !important}
      ...
      <section id="validation" style={{maxWidth:'none'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'1.1rem',marginTop:'1.6rem'}}>
          {MEASUREMENTS.map(([name, desc]) => (
            <div key={name} className="val-row" style={{display:'grid',gridTemplateColumns:'minmax(200px,260px) 1fr',gap:'1.6rem',
      ...
      <footer>
        ...
        <span style={{display:'flex',gap:'1.25rem',flexWrap:'wrap',justifyContent:'center',margin:'0.75rem 0',fontSize:'13px'}}>
          <a href="/verify" style={{color:'var(--green)'}}>Verify a manifest</a>
          <a href="/certified" style={{color:'var(--green)'}}>Certified access</a>
          <a href="/reports" style={{color:'var(--green)'}}>Reports</a>
        </span>
<span className="footer-copy" style={{whiteSpace:'normal',fontSize:'10.25px',flexBasis:'100%',textAlign:'justify',textAlignLast:'justify'}}>© 2026 Emilio R. Bruno · Aether-Lang.org Inc. (CBCA federal) · Kamloops, BC, Canada · Five US provisional patents filed with the USPTO (2026); CA in preparation · AI assistance (Claude/Anthropic) disclosed · Page updated 2026-09-18</span>      </footer>
    </>
  )
}
